...
class RLNode(Node):
    def __init__(self, cfg_hydra: DictConfig, output_dir):
        ...
        try:
            obs_norm_transform.init_stats(num_iter=2000, reduce_dim=0, cat_dim=0)
        ...
        def get_obs_norm_stats():
            try:
                obs_norm_mean = torch.as_tensor(obs_norm_transform.loc, dtype=torch.float32).reshape(-1)
                obs_norm_std = torch.as_tensor(obs_norm_transform.scale, dtype=torch.float32).reshape(-1)
                if obs_norm_mean.numel() != obs_dim or obs_norm_std.numel() != obs_dim:
                    raise RuntimeError("ObservationNorm stats shape mismatch")
                return True, obs_norm_mean, torch.clamp(obs_norm_std, min=1e-6)
            except Exception:
                return False, torch.zeros(obs_dim, dtype=torch.float32), torch.ones(obs_dim, dtype=torch.float32)

        cfg_snapshot = OmegaConf.to_container(cfg_hydra, resolve=True)

        def build_checkpoint(step_value):
            ...
            return {
                ...
                "training_config": cfg_snapshot,
            }

        def write_checkpoint_metadata_sidecar(path, step_value):
            obs_norm_enabled, _, _ = get_obs_norm_stats()
            metadata = {
                "step": int(step_value),
                "env_id": selected_env_id,
                "obs_dim": int(obs_dim),
                ...
                "obs_norm_enabled": obs_norm_enabled,
                "obs_norm": {
                    "mean": "from ObservationNorm.loc",
                    "std": "from ObservationNorm.scale",
                },
                "notes": "Observation normalization stats saved for train/inference reproducibility."
            }
            with open(path + ".meta.json", "w", encoding="utf-8") as metadata_file:
        ...
        for i, tensordict_data in enumerate(collector):
            ...
            if global_step % 10_000 < batch_size:
                ...
                torch.save(build_checkpoint(global_step), checkpoint_path)
                write_checkpoint_metadata_sidecar(checkpoint_path, global_step)
            """
    ...
    def listener_callback(self, msg):
        ...
        if(msg.name[0] == ('Joint '+self.id)):
            ...
            msgMd80ExoPos = msg.position  # Save the position value (float[]) from the message
            msgMd80ExoAcc = (msg.velocity[0] - msgMd80ExoVel[0])/(1/msgMd80ExoHz)
            msgMd80ExoVel = msg.velocity
            msgMd80ExoEffort = msg.effort
...
def evaluate_agent(policy, env, num_episodes=5, device="cpu"):
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            ...
            motor_obs = [msgMd80ExoPos[0], msgMd80ExoVel[0], msgMd80ExoAcc[0], msgMd80ExoEffort[0]] #May cause problems
...
def main(cfg_hydra: DictConfig):
    ...
    rclpy.init()
    rl_node = RLNode(cfg_hydra, output_dir)
    print("RL node created, do we get to spin it?")
    rclpy.spin(rl_node)
