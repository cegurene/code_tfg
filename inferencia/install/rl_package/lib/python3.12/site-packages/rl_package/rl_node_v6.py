"""RL_NODE_V6 - Reinforcement Learning Agent with Adjustable Torque Scaling
...
class MinimalPublisher(Node):
    ...
    TORQUE_SCALE =  0.5 # <-- CAMBIAR ESTE VALOR Y RECOMPILAR
    ...
    SENS_DIAG_ENABLED = True
    SENS_DIAG_PERIOD = 100  # callbacks
    SENS_EPS_POS = 0.01  # rad
    SENS_EPS_VEL = 0.05  # rad/s
    SENS_EPS_ACC = 0.50  # rad/s^2
    ...
    def _configure_runtime_mode(self):
        ...
        self.SENS_DIAG_ENABLED = self._env_bool('RL_SENS_DIAG_ENABLED', self.SENS_DIAG_ENABLED)

        env_sens_period = os.getenv('RL_SENS_DIAG_PERIOD')
        if env_sens_period is not None:
            try:
                self.SENS_DIAG_PERIOD = max(1, int(env_sens_period))
            except ValueError:
                self.get_logger().warn(f"Invalid RL_SENS_DIAG_PERIOD='{env_sens_period}', using default.")
    ...
    def __init__(self):
        ...
        print(f"Sensitivity diag:       {self.SENS_DIAG_ENABLED} (period={self.SENS_DIAG_PERIOD})")
        ...
        print("")
        print("Para cambiar el escalado: Editar rl_node_v6.py línea ~40")
        print("Valores sugeridos: 0.09 (muy bajo), 1.0 (medio), 5.0 (alto), 9.0 (máximo)")
        print("="*70 + "\n")
        
        # Interactive angle limit calibration
        # Interactive angle limit calibration (only if safety limits are enabled)
        if self.ENABLE_SAFETY_LIMITS:
            self._calibrate_angle_limits()
        else:
            self.angle_min = -1e9
            self.angle_max = 1e9
            self.get_logger().warn("Safety limits DISABLED: skipping angle calibration.")
        ...
        self.get_logger().info('rl_node_v5 iniciado (con escalado de torque)')
        ...
        # Optional actuator self-test to verify hardware path independent of inference
    ...
    def _publish_torque(self, torque_value):
        ...
        if self.hardware_publisher_ is not None:
            ...
            msg_hw.target_torque = [torque_clamped]
    ...
    def _predict_normalized_action(self, observations):
        ...
        return action_tensor[0].item() if action_tensor.numel() > 0 else 0.0

    def _compute_local_sensitivity(self, observations):
        if self.model is None:
            return None

        obs_base = numpy.array(observations, dtype=numpy.float32)
        if obs_base.ndim != 1:
            return None

        specs = [
            ("pos", 7, self.SENS_EPS_POS),
            ("vel", 23, self.SENS_EPS_VEL),
            ("acc", 39, self.SENS_EPS_ACC),
        ]

        derivatives = {}
        for label, idx, eps in specs:
            if idx >= len(obs_base) or eps <= 0.0:
                derivatives[label] = None
                continue

            obs_plus = obs_base.copy()
            obs_minus = obs_base.copy()
            obs_plus[idx] += eps
            obs_minus[idx] -= eps

            tau_plus = -self._predict_normalized_action(obs_plus) * self.TORQUE_SCALE
            tau_minus = -self._predict_normalized_action(obs_minus) * self.TORQUE_SCALE
            derivatives[label] = (tau_plus - tau_minus) / (2.0 * eps)
    ...
    def listener_callback(self, msg):
        ...
        if self.model is None:
            ...
            act_normalized = self._predict_normalized_action(observations)
        ...
        if self._callback_count % 50 == 1 and self.model is not None:
            self.get_logger().info(
        ...
        if self.ENABLE_SAFETY_LIMITS and self.APPLY_TORQUE and not self._e_stop_latched and self.current_pos is not None:
            ...
            out_of_calibrated_range = (
                pos_now < (self.angle_min - self.LIMIT_MARGIN_RAD)
                or pos_now > (self.angle_max + self.LIMIT_MARGIN_RAD)
            )
            over_velocity = abs(vel_now) > self.MAX_SAFE_VEL_RAD_S
            abs_pos_overflow = abs(pos_now) > self.ABS_POS_HARD_LIMIT_RAD

            if out_of_calibrated_range or over_velocity or abs_pos_overflow:
                ...
                self.get_logger().error(
                    ...
                    f"range=[{self.angle_min:.3f},{self.angle_max:.3f}] margin={self.LIMIT_MARGIN_RAD:.3f}"
                )
        
        # DEBUG: Log actual command being sent
        if self._callback_count % 50 == 1:
        ...
        # Variability diagnostics every 100 callbacks (~50s at 2Hz, ~10s at 10Hz)
        if self._callback_count % 100 == 1 and len(self._diag_window) >= 20:
        ...
        if (
            ...
            and self.SENS_DIAG_ENABLED
            and self._callback_count % self.SENS_DIAG_PERIOD == 1
        ):
            sens = self._compute_local_sensitivity(observations)
            if sens is not None:
                pos_txt = f"{sens['pos']:+.4f} Nm/rad" if sens.get('pos') is not None else 'n/a'
                vel_txt = f"{sens['vel']:+.4f} Nm/(rad/s)" if sens.get('vel') is not None else 'n/a'
                acc_txt = f"{sens['acc']:+.4f} Nm/(rad/s²)" if sens.get('acc') is not None else 'n/a'
                self.get_logger().info(
                    f'🧪 SENS: dTau/dPos={pos_txt} | dTau/dVel={vel_txt} | dTau/dAcc={acc_txt}'
                )
        ...
        # Log
        if self._callback_count % 50 == 1 or safety_override:
