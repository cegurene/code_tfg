#!/usr/bin/env python3
"""Sim2Real runner for hip motor tracking using pyCandle impedance mode.
...
layer to communicate directly with pyCandle MD80 commands in IMPEDANCE mode.

If impedance gains are set to zero (`kp=0`, `kd=0`), torque command behaves as
pure torque through `setTargetTorque()`.
"""
...
class PyCandleImpedanceMotorInterface:
    """Drop-in replacement for Ros2MotorInterface using pyCandle IMPEDANCE mode."""
    ...
    # Impedance gains fixed at 0 to simulate pure torque mode behavior via IMPEDANCE interface.
    ...
    def __init__(
        ...
        self.candle.controlMd80Mode(self.drive, pyCandle.IMPEDANCE)
        self.candle.controlMd80Enable(self.drive, True)
        # Set impedance gains to 0 to achieve pure torque mode behavior through IMPEDANCE interface.
        # This allows testing hardware behavior without ROS2 while maintaining the same control loop.
        self.drive.setImpedanceControllerParams(0.0, 0.0)
        ...
        if self._verbose:
            print(
                (
                    "pyCandle impedance interface ready: motor_id={} baud={} fdcan={} "
                    "kp=0 kd=0 (pure torque mode) max_torque={}Nm"
                ).format(
                    self.motor_id,
                    self._baud_label,
                    self._fdcan_enabled,
...
def _build_parser():
    ...
    parser.description = (
        "Sim2Real hip runner using pyCandle IMPEDANCE transport for REAL mode."
    )
    parser.set_defaults(
...
def main() -> int:
    ...
    PyCandleImpedanceMotorInterface.configure_from_args(args)
    base.Ros2MotorInterface = PyCandleImpedanceMotorInterface
