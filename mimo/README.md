...
## Knee Motor Control Scripts

Detailed knee motor control documentation (commands, flags, outputs, safety notes) is available in:

- [mimoEnv/README.md](mimoEnv/README.md)

#### Notas de Tuning

1. **Estabilidad**: Si el control oscila mucho, aumenta `--joint-damping-scale` o `--joint-stiffness-scale`
2. **Velocidad de respuesta**: Si la respuesta es muy lenta, reduce `--joint-armature-scale` o `--joint-damping-scale`
3. **Realismo**: Combina múltiples parámetros para simular condiciones fisiológicas realistas
4. **Debugging**: Empieza con un parámetro a la vez para entender su efecto, luego combina
